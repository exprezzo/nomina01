var EdicionPercepciones_nomina = function(){
	this.editado=false;
	this.tituloNuevo='Nueva Percepcion';
	this.saveAndClose=false;
	
	this.configurarComboFk_TipoPercepcion=function(){
		var me=this;
		
		$('select[name="fk_TipoPercepcion"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_TipoPercepcion input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_TipoPercepcionEnAjax) return true;			
			me.setDSFk_TipoPercepcion();
			me.Fk_TipoPercepcionEnAjax=true;
		 });
	};
		
		
	this.setDSFk_TipoPercepcion = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/percepciones_nomina/buscarTipo_percepcion',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_TipoPercepcion"]').wijcombobox('option','data',datasource);
	};
		
	
	this.borrar=function(){		
		var r=confirm("¿Eliminar Percepcion?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Percepcion: ' + getValorCampo('Concepto'));
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_TipoPercepcion"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_TipoPercepcion"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_TipoPercepcion'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_TipoPercepcion'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_TipoPercepcion'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboFk_TipoPercepcion();
	};
