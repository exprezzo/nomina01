var EdicionHoras_extra_nomina = function(){
	this.editado=false;
	this.tituloNuevo='Nueva Hora Extra';
	this.saveAndClose=false;
	
	this.configurarComboFk_TipoHoras=function(){
		var me=this;
		
		$('select[name="fk_TipoHoras"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_TipoHoras input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_TipoHorasEnAjax) return true;			
			me.setDSFk_TipoHoras();
			me.Fk_TipoHorasEnAjax=true;
		 });
	};
		
		
	this.setDSFk_TipoHoras = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/horas_extra_nomina/buscarTipo_hora',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_TipoHoras"]').wijcombobox('option','data',datasource);
	};
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Hora Extra');
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_TipoHoras"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_TipoHoras"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_TipoHoras'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_TipoHoras'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_TipoHoras'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboFk_TipoHoras();
	};
