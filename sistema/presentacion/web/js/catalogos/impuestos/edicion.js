var EdicionImpuestos = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Impuesto';
	this.saveAndClose=false;
	
	this.configurarComboFk_naturaleza=function(){
		var me=this;
		
		$('select[name="fk_naturaleza"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_naturaleza input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_naturalezaEnAjax) return true;			
			me.setDSFk_naturaleza();
			me.Fk_naturalezaEnAjax=true;
		 });
	};
		
		
	this.setDSFk_naturaleza = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/impuestos/buscarTipo_de_impuesto',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_naturaleza"]').wijcombobox('option','data',datasource);
	};
		
	
	this.borrar=function(){		
		var r=confirm("¿Eliminar Impuesto?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Impuesto: ' + getValorCampo('nombre') + ''); 
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_naturaleza"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_naturaleza"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_naturaleza'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_naturaleza'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_naturaleza'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboFk_naturaleza();
	};
