var EdicionDeducciones_nomina = function(){
	this.editado=false;
	this.tituloNuevo='Nueva Deducci贸n';
	this.saveAndClose=false;
	
	this.configurarComboFk_TipoDeduccion=function(){
		var me=this;
		
		$('select[name="fk_TipoDeduccion"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_TipoDeduccion input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_TipoDeduccionEnAjax) return true;			
			me.setDSFk_TipoDeduccion();
			me.Fk_TipoDeduccionEnAjax=true;
		 });
	};
		
		
	this.setDSFk_TipoDeduccion = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/deducciones_nomina/buscarTipo_deduccion',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_TipoDeduccion"]').wijcombobox('option','data',datasource);
	};
		
	
	this.borrar=function(){		
		var r=confirm("驴Eliminar Deducci贸n?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Deducci贸n: ' + getValorCampo('Concepto'));
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_TipoDeduccion"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_TipoDeduccion"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_TipoDeduccion'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_TipoDeduccion'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_TipoDeduccion'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboFk_TipoDeduccion();
	};
