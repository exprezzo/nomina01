var EdicionConceptos_para_nomina = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Concepto Para Nomina';
	this.saveAndClose=false;
	
	this.configurarComboFk_unidad=function(){
		var me=this;
		
		$('select[name="fk_unidad"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_unidad input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_unidadEnAjax) return true;			
			me.setDSFk_unidad();
			me.Fk_unidadEnAjax=true;
		 });
	};
		
		
	this.setDSFk_unidad = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/conceptos_para_nomina/buscarUnidad_de_medida',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_unidad"]').wijcombobox('option','data',datasource);
	};
		
	
	this.borrar=function(){		
		var r=confirm("¿Eliminar Concepto Para Nomina?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Concepto Para Nomina: ' + getValorCampo('nombre') + ''); 
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_unidad"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_unidad"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_unidad'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_unidad'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_unidad'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
