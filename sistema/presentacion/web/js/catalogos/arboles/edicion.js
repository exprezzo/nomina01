var EdicionArboles = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Nodo';
	this.saveAndClose=false;
	
	this.configurarComboParent=function(){
		var me=this;
		
		$('select[name="parent"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_parent input[role="textbox"]').bind('keypress', function(){			
			if (me.ParentEnAjax) return true;			
			me.setDSParent();
			me.ParentEnAjax=true;
		 });
	};
		
		
	this.setDSParent = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/arboles/buscarArbol',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="parent"]').wijcombobox('option','data',datasource);
	};
		
	
	this.borrar=function(){		
		var r=confirm("¿Actualizar Nodo?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Nodo: ' + getValorCampo('title') + ''); 
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="parent"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="parent"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['parent'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['parent'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['parent'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
