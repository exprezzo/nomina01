var EdicionImpuestos_de_nomina = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Impuesto De Nomina';
	this.saveAndClose=false;
	
	this.configurarComboFk_impuesto=function(){
		var me=this;
		
		$('select[name="fk_impuesto"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_impuesto input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_impuestoEnAjax) return true;			
			me.setDSFk_impuesto();
			me.Fk_impuestoEnAjax=true;
		 });
	};
		
		
	this.setDSFk_impuesto = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/impuestos_de_nomina/buscarImpuesto',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_impuesto"]').wijcombobox('option','data',datasource);
	};
		
	this.configurarComboFk_tipo_impuesto=function(){
		var me=this;
		
		$('select[name="fk_tipo_impuesto"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_tipo_impuesto input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_tipo_impuestoEnAjax) return true;			
			me.setDSFk_tipo_impuesto();
			me.Fk_tipo_impuestoEnAjax=true;
		 });
	};
		
		
	this.setDSFk_tipo_impuesto = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/impuestos_de_nomina/buscarTipo_de_impuesto',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_tipo_impuesto"]').wijcombobox('option','data',datasource);
	};
		
	
	this.borrar=function(){		
		var r=confirm("¿Eliminar Impuesto De Nomina?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Impuesto De Nomina: ' + getValorCampo('nombre') + ''); 
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_impuesto"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_impuesto"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_impuesto'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_impuesto'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_impuesto'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_tipo_impuesto"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_tipo_impuesto"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_tipo_impuesto'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_tipo_impuesto'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_tipo_impuesto'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboFk_impuesto();
this.configurarComboFk_tipo_impuesto();
	};
